...
public static class Auth0ConfigHelper
{
    ...
    public static Auth0Settings GetRequiredAuth0Settings(IConfiguration config)
    {
        ...
        // New: Extract and validate Namespace for custom claims
        var ns = config["Auth0:Namespace"]
            ?? throw new InvalidOperationException("Auth0:Namespace is missing in configuration.");

        return new Auth0Settings(authority, clientId, audience, ns);
    }
}